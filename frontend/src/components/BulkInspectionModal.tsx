...
const BulkInspectionModal: React.FC<Props> = ({ assetPids, onClose, onSaved }) => {
  const today = useMemo(() => new Date().toISOString().slice(0, 10), []);

  const [inspectionType, setInspectionType] = useState<InspectionType>('일반점검');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await Promise.all(assetPids.map((pid) => assetsApi.addInspection(pid, params)));
      onSaved();
    } catch {
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <h2 className="font-semibold text-gray-900">
            다건 점검 등록 <span className="text-indigo-600">({assetPids.length}개)</span>
          </h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <p className="text-xs text-gray-500 bg-gray-50 px-3 py-2 rounded-lg">
            선택한 {assetPids.length}개 자산에 동일한 점검 내용이 각각 등록됩니다.
          </p>
          ...
          <div className="flex gap-2 pt-1">
            ...
            >
              {saving ? '등록 중...' : `${assetPids.length}개 등록`}
            </button>
          </div>
        </form>
      </div>
...
export default BulkInspectionModal;

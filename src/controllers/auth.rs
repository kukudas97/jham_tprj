...
async fn login(State(ctx): State<AppContext>, Json(params): Json<LoginParams>) -> Result<Response> {
    ...
    let company_pid = fetch_company_pid(&ctx, user.company_id).await;
    format::json(LoginResponse::new(&user, &token, company_pid))
}
...
async fn current(auth: auth::JWT, State(ctx): State<AppContext>) -> Result<Response> {
    ...
    let company_pid = fetch_company_pid(&ctx, user.company_id).await;
    format::json(CurrentResponse::new(&user, company_pid))
}

async fn fetch_company_pid(ctx: &AppContext, company_id: Option<i32>) -> Option<String> {
    let id = company_id?;

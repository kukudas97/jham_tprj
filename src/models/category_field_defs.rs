use loco_rs::prelude::*;
use uuid::Uuid;

pub use super::_entities::category_field_defs::{self, ActiveModel, Entity, Model};

#[async_trait::async_trait]
impl ActiveModelBehavior for ActiveModel {
    async fn before_save<C>(self, _db: &C, insert: bool) -> Result<Self, DbErr>
    where
        C: ConnectionTrait,
    {
        if insert {
            let mut this = self;
            this.pid = ActiveValue::Set(Uuid::new_v4());
            this.created_at = ActiveValue::Set(chrono::Utc::now().into());
            this.updated_at = ActiveValue::Set(chrono::Utc::now().into());
            Ok(this)
        } else {
            let mut this = self;
            if this.updated_at.is_unchanged() {
                this.updated_at = ActiveValue::Set(chrono::Utc::now().into());
            }
            Ok(this)
        }
    }
}

impl Model {
    pub async fn find_by_company(
        db: &DatabaseConnection,
        company_id: i32,
    ) -> ModelResult<Vec<Self>> {
        use sea_orm::QueryOrder;
        Ok(Entity::find()
            .filter(
                model::query::condition()
                    .eq(category_field_defs::Column::CompanyId, company_id)
                    .build(),
            )
            .order_by_asc(category_field_defs::Column::SortOrder)
            .all(db)
            .await?)
    }

    pub async fn find_by_category(
        db: &DatabaseConnection,
        category_id: i32,
    ) -> ModelResult<Vec<Self>> {
        use sea_orm::QueryOrder;
        Ok(Entity::find()
            .filter(
                model::query::condition()
                    .eq(category_field_defs::Column::CategoryId, category_id)
                    .build(),
            )
            .order_by_asc(category_field_defs::Column::SortOrder)
            .all(db)
            .await?)
    }

    pub async fn find_by_pid_and_company(
        db: &DatabaseConnection,
        pid: &str,
        company_id: i32,
    ) -> ModelResult<Self> {
        let uuid = Uuid::parse_str(pid).map_err(|e| ModelError::Any(e.into()))?;
        let item = Entity::find()
            .filter(
                model::query::condition()
                    .eq(category_field_defs::Column::Pid, uuid)
                    .eq(category_field_defs::Column::CompanyId, company_id)
                    .build(),
            )
            .one(db)
            .await?;
        item.ok_or_else(|| ModelError::EntityNotFound)
    }
}

impl ActiveModel {}
impl Entity {}

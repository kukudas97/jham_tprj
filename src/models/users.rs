...
impl Model {
    ...
    pub async fn create_with_password(
        ...
        let company_id = if let Some(code) = &params.company_code {
            let uuid = Uuid::parse_str(code)
                .map_err(|_| ModelError::msg("유효하지 않은 초대 코드입니다."))?;
            let company = companies::Entity::find()
                .filter(companies::Column::Pid.eq(uuid))
                .one(&txn)
                .await?
                .ok_or_else(|| ModelError::msg("초대 코드에 해당하는 회사를 찾을 수 없습니다."))?;
            company.id
        } else if let Some(name) = &params.company_name {
            ...
            return Err(ModelError::msg("회사명 또는 초대 코드를 입력해주세요."));
        };
        ...
        let user = users::ActiveModel {
            ...
            company_id: ActiveValue::set(Some(company_id)),
            ..Default::default()
        }
        .insert(&txn)
        .await?;
